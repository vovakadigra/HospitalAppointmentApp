...
def save_info():
    ...
    if(date_info == ""):
        messagebox.showwarning(title="Error",message="Warning: Enter a valod Date of Birth ")
        return -1
...
heading = Label(text="Registration", bg="#140D4F", fg="#FFFFFF",font='Times 15', width="500", height="4")
heading.pack()

today = date.today()
todayfr = today.strftime("%d-%m-%Y")
cal = Calendar(screen,background='#140D4F',date_pattern='dd-mm-yyyy', selectmode = 'day',
               year = 2022, month = 5,
               day = 10)
...
myCombo_country = ttk.Combobox(screen,state="readonly",value=option_country,width="27")
...
firstname_text.place(x=15, y=110)
lastname_text.place(x=15, y=180)
country_text.place(x=15, y=320)
phone_text.place(x=15, y=250)
age_text.place(x=280,y=180)
password_text.place(x=280,y=110)
...
phone_entry = Entry(textvariable=phone, width="30",fg="grey")
...
firstname_entry.place(x=15, y=140)
lastname_entry.place(x=15, y=210)
myCombo_country.place(x=15, y=350)
phone_entry.place(x=15, y=280)
password_entry.place(x=280, y=140)
cal.place(x=280, y=210,width="200",height="160" )
R1.place(x=30, y=380)
R2.place(x=100,y=380)


btn=ttk.Button(screen, text="Register", command=print_selected)
